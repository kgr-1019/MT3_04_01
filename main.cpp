...
void DrawSphere(const Sphere& sphere, const Matrix4x4& viewProjectionMatrix, const Matrix4x4& viewportMatrix)
{
	...
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex)
	{
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex)
		{
			...
			// ワールド座標系での頂点を求める
			a = {
				(sphere.radius) * std::cos(lat) * std::cos(lon) + sphere.position.x,
				(sphere.radius) * std::sin(lat) + sphere.position.y,
				(sphere.radius) * std::cos(lat) * std::sin(lon) + sphere.position.z
			};

			b = {
				(sphere.radius) * std::cos(lat + kLatEvery) * std::cos(lon) + sphere.position.x,
				(sphere.radius) * std::sin(lat + kLatEvery) + sphere.position.y,
				(sphere.radius) * std::cos(lat + kLatEvery) * std::sin(lon) + sphere.position.z
			};

			c = {
				(sphere.radius) * std::cos(lat) * std::cos(lon + kLonEvery) + sphere.position.x,
				(sphere.radius) * std::sin(lat) + sphere.position.y,
				(sphere.radius) * std::cos(lat) * std::sin(lon + kLonEvery) + sphere.position.z
			};
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		// 角速度
		sphere.position.x = circleCenter.x * std::cos(angle) * circleRadius;
		sphere.position.y = circleCenter.y * std::cos(angle) * circleRadius;
		sphere.position.z = circleCenter.z;

		// 等速円運動
		sphere.velocity = {
			-circleRadius * float(M_PI) * std::sin(angle),
			circleRadius * float(M_PI) * std::cos(angle),
			0,
		};

		// 等速円運動の加速度
		sphere.acceleration = {
			-((float(M_PI) * float(M_PI)) * (sphere.position.x - circleCenter.x)),
			-((float(M_PI) * float(M_PI)) * (sphere.position.y - circleCenter.y)),
